...
const DashboardLayout = ({ children }: DashboardLayoutProps) => {
  // The local 'activeLink' state is no longer needed.
  // NavLink will handle the active state automatically based on the current URL.

  return (
    <div className="min-h-screen bg-storiq-dark flex">
      {/* Sidebar */}
      <aside className="w-64 bg-storiq-dark border-r border-storiq-border flex flex-col flex-shrink-0">
        {/* Logo */}
        <div className="p-6">
          ...
          <div className="space-y-2">
            {sidebarItems.map((item) => (
              <NavLink
                key={item.label}
                to={item.href}
                end // Use 'end' for the Home link to prevent it from matching other routes
                className={({ isActive }) =>
                  cn(
